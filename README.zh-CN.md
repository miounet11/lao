...
你已经登录了微博、知乎、B站、小红书、Twitter、GitHub、LinkedIn — iatlas-browser 让 AI Agent **直接用你的登录态**。

```bash
iatlas-browser site twitter/search "AI agent"       # 搜索推文
iatlas-browser site zhihu/hot                        # 知乎热榜
iatlas-browser site arxiv/search "transformer"       # 搜论文
iatlas-browser site eastmoney/stock "茅台"            # 实时股票行情
iatlas-browser site boss/search "AI 工程师"           # 搜职位
iatlas-browser site wikipedia/summary "Python"       # 维基百科摘要
iatlas-browser site youtube/transcript VIDEO_ID      # YouTube 字幕全文
...
iatlas-browser 翻转了这个逻辑：**不是让网站适配机器，而是让机器使用人的界面。** adapter 在你的浏览器 tab 里跑 `eval`，用你的 Cookie 调 `fetch()`，或者直接调用页面的 webpack 模块。网站以为是你在操作。因为**就是你**。

| | Playwright / Selenium | 爬虫库 | iatlas-browser |
|---|---|---|---|
...
```bash
iatlas-browser site update    # 拉取 97 个社区适配器
iatlas-browser site list      # 看看有什么
iatlas-browser site zhihu/hot # 开搞
```
...
没有 iatlas-browser，AI Agent 的世界是：**文件系统 + 终端 + 少数有 API key 的服务。**

有了 iatlas-browser：**文件系统 + 终端 + 整个互联网。**
...
iatlas-browser snapshot -i                # 可访问性树
iatlas-browser click @3                   # 点击元素
iatlas-browser fill @5 "hello"            # 填写输入框
iatlas-browser eval "document.title"      # 执行 JS
iatlas-browser fetch URL --json           # 带登录态的 fetch
iatlas-browser network requests --with-body --json  # 抓包
iatlas-browser screenshot                 # 截图
```
...
AI Agent (Claude Code, Codex, Cursor 等)
       │ CLI 或 MCP (stdio)
       ▼
iatlas-browser CLI ──HTTP──▶ Daemon ──SSE──▶ Chrome 扩展
                                              │
                                              ▼ chrome.debugger (CDP)
                                         你的真实浏览器
```
